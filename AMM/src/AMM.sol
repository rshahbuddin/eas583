...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		if (sellToken == tokenA) {
			...
			require(swapAmt > 0, "Zero output");
			require(ERC20(tokenB).transfer(msg.sender, swapAmt), "Transfer failed");
		} else {
			uint256 newQtyB = qtyB + effectiveSell;
			uint256 newQtyA = invariant / newQtyB;
			swapAmt = qtyA - newQtyA;
			require(swapAmt > 0, "Zero output");
			require(ERC20(tokenA).transfer(msg.sender, swapAmt), "Transfer failed");
		}
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
		...
		if (amtA > 0) {
			require(ERC20(tokenA).transferFrom(msg.sender, address(this), amtA), "Transferring tokenA failed");

		}
		if (amtB > 0) {
			require(ERC20(tokenB).transferFrom(msg.sender, address(this), amtB), "Transferring tokenB failed");
		}
